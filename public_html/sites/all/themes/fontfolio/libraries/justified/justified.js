...
(function($){
   ...
   $.fn.justifiedGallery = function(options){
      ...
      if(settings.captions)
        var fullImage = image['src'];
        fullImage = fullImage.replace('styles/1124xvar/public/', '');
        fullImage = encodeURIComponent(fullImage);
        //console.log(image);
        var imageFid = image['title'];
        var dl = '<a href="/sites/all/themes/fontfolio/download.php?path=' + fullImage + '.jpg" class="download-link" title="Download original file">Download Original</a>';
        var like = '<a class="plusoneapi-votes" id="plusoneapi-votes-' + imageFid + '" rel="' + imageFid + '" title="Plus 1"><span class="heart">Votes:</span><span class="count"></span></a>';
				ris += "  <div style=\"bottom:" + (nh - minRowHeight) + "px;\" class=\"jg-image-label\">" + dl + like + "</div>";
      ...
      jQuery('.plusoneapi-votes').click(function() {
        ...
        jQuery(this).children('.heart').fadeTo(100, 0.33, function() {
          jQuery(this).fadeTo(100, 0.99, function() {
            jQuery(this).fadeTo("slow", 0.66, function() {
              jQuery(this).fadeTo("fast", 0.99);
            });
          })
        });
      });
      jQuery('.plusoneapi-votes').each(function() {
        var id = jQuery(this).attr('rel');
